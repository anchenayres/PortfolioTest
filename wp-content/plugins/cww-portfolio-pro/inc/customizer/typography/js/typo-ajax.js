jQuery(document).ready(function($) {
  "use strict"; 
    ...
    $.ajax({
        url: ajaxurl,
        data: ({
            'action': 'cww_portfolio_pro_get_google_font_variants',
            'font_family': font_family,
        }),
        success: function(response) {
            var jsonDecode = JSON.parse(response);
          
            dis.parent('.typography-font-family').siblings('.typography-font-weight').children('select').html(jsonDecode.message1);            
            dis.parent('.typography-font-family').siblings('.typography-font-style').children('select').html(jsonDecode.message2);  
