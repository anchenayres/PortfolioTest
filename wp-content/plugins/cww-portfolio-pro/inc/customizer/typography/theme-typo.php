...
function cww_portfolio_pro_get_google_font_variants(){
	
    $cww_portfolio_pro_font_list 	= get_option( 'cww_portfolio_pro_google_fonts_val','Lato' );
    $font_family 			= $_REQUEST['font_family'];
    $font_array 			= cww_portfolio_pro_search_key($cww_portfolio_pro_font_list,'family', $font_family);
    ...
    foreach ($variants_array  as $key => $variants ) {
    	...
    	if( is_numeric($variants)){
    		$options_array .= '<option value="'.esc_attr($variants).'">'.esc_html($variants).'</option>';	
    	}
    	if( ctype_alpha($variants) ){
    		$options_array_string .= '<option value="'.esc_attr($variants).'">'.esc_html($variants).'</option>';
    	}
	...
	if( !empty($options_array || $options_array_string) ){
		echo  json_encode( array(
                   "message1" => $options_array,
                   "message2" => $options_array_string,
                   ));
...
add_action("wp_ajax_cww_portfolio_pro_get_google_font_variants", "cww_portfolio_pro_get_google_font_variants");

function cww_portfolio_pro_search_key($array, $key, $value){
    $results = array();
    if (is_array($array)) {
        ...
        foreach ($array as $subarray) {
            $results = array_merge($results, cww_portfolio_pro_search_key($subarray, $key, $value));
        }
    }
    return $results;
}
...
*/
if ( ! function_exists( 'cww_portfolio_pro_store_typhography' ) ) {
	function cww_portfolio_pro_store_typhography(){
...
if($h6_font_family ==''){
	$h6_font_family = 'Open Sans';
		...
		wp_enqueue_style('cww_portfolio_pro-typhography-font', add_query_arg($query_args, "//fonts.googleapis.com/css"));
	}
}
add_action('wp_enqueue_scripts','cww_portfolio_pro_store_typhography');
